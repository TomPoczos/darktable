...
__kernel void
blackwhite (read_only image2d_t in, write_only image2d_t out, const int width, const int height,
           const float4 grey)
{
  const int x = get_global_id(0);
  const int y = get_global_id(1);
  ...
  const float grey_mix = fmax(dot(pixel.xyz, grey.xyz), 0.0f);
  float4 opixel = (float4)(grey_mix, grey_mix, grey_mix, pixel.w);
